...
vector<EventProcessor *> DetectorDriverXmlParser::ParseProcessors(const pugi::xml_node &node) {
    ...
    for (pugi::xml_node processor = node; processor; processor = processor.next_sibling(node.name())) {
		...
		pugi::xml_node frontposcorrection = processor.child("FrontPositionCorrection");
		double frontmeanerg = 1.0;
		double frontconstant = 0.0;
		double frontslope = 0.0;
		if( frontposcorrection ){
			frontmeanerg = frontposcorrection.attribute("EnergyMean").as_double(1.0);
			frontconstant = frontposcorrection.attribute("constant").as_double(0.0);
			frontslope = frontposcorrection.attribute("slope").as_double(0.0);
		}
		pugi::xml_node backposcorrection = processor.child("BackPositionCorrection");
		double backmeanerg = 1.0;
		double backconstant = 0.0;
		double backslope = 0.0;
		if( backposcorrection ){
			backmeanerg = backposcorrection.attribute("EnergyMean").as_double(1.0);
			backconstant = backposcorrection.attribute("constant").as_double(0.0);
