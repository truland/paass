...
bool RootDevProcessor::Process(RawEvent &event) {
	...
	for (auto it = Events.begin(); it != Events.end(); it++) {
		...
		if ((*it)->GetTrace().size() > 0) {
			...
			RDstruct.hasValidTimingAnalysis = trace.HasValidTimingAnalysis();
			RDstruct.hasValidWaveformAnalysis = trace.HasValidWaveformAnalysis();
			RDstruct.baseline = trace.GetBaselineInfo().first;
			RDstruct.stdBaseline = trace.GetBaselineInfo().second;
			RDstruct.trace = trace;
			RDstruct.maxPos = trace.GetMaxInfo().first;
			RDstruct.maxVal = trace.GetMaxInfo().second;
			RDstruct.extMaxVal = trace.GetExtrapolatedMaxInfo().second;
			RDstruct.tqdc = trace.GetQdc();
			RDstruct.highResTime = (*it)->GetHighResTimeInNs();
			if (Rev == "F") {
				RDstruct.phase = trace.GetPhase() * Globals::get()->GetAdcClockInSeconds((*it)->GetChanID().GetModFreq()) * 1e9;
			} else {
				RDstruct.phase = trace.GetPhase() * Globals::get()->GetAdcClockInSeconds() * 1e9;
			}
		}
		if (!(*it)->GetQdc().empty()) {
