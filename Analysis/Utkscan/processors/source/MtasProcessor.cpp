...
void MtasProcessor::DeclarePlots(void){
	...
	DeclareHistogram2D(DD_MTAS_C1_POSITION,SA,SA,"Mtas C1 vs Position");
...
bool MtasProcessor::PreProcess(RawEvent &event) {
	...
	for (auto segIter = MtasSegVec.begin(); segIter != MtasSegVec.end(); ++segIter) {
		...
		if( segIter->IsValidSegment() ){
			...
			if (segmentID >= 0 && segmentID <= 5){
				...
				double position = ((SA/2)*(1.0 + segIter->GetSegPosition()));
	...
	//Put MTASThings into PlaceDetector for other classes like MTASImplant and BSM
	//TreeCorrelator::get()->place("MTAS_Total").activate(EventData(MTASFirstTime,MTASTotal));
	//TreeCorrelator::get()->place("MTAS_Center").activate(EventData(MTASFirstTime,MTASCenter));
	//TreeCorrelator::get()->place("MTAS_Inner").activate(EventData(MTASFirstTime,MTASInner));
	//TreeCorrelator::get()->place("MTAS_Middle").activate(EventData(MTASFirstTime,MTASMiddle));
	//TreeCorrelator::get()->place("MTAS_Outer").activate(EventData(MTASFirstTime,MTASOuter));

	//for( unsigned int ii = 0; ii < MtasSegVec.size(); ++ii ){
	//	string name = "MTAS_Segment_" + to_string(ii);
	//	TreeCorrelator::get()->place(name).activate(EventData(MTASFirstTime,MtasSegVec.at(ii).GetSegAvgEnergy()));
	//}
